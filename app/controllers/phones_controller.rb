class PhonesController < ApplicationController
  ...
  def update
    ...
    respond_to do |format|
      if @phone.update_attributes(params[:phone])
        format.html { redirect_to(contact_path(@phone.contact_id),  :notice => 'Phone was successfully updated.') }
        format.xml  { head :ok }
      else
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to(user_path(Phone.find_by_id(@phone.user_id)))}
      format.xml  { head :ok }
    end
  end
end
